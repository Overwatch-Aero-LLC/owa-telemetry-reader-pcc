...
// File modified by:             //
// Overwatch Aero, LLC           //
// 1248 Alamo Pintado Rd.        //
// Solvang, CA 93463             //
// (832) 971-1302     (voice)    //
// http://www.overwatchaero.com  //
///////////////////////////////////

////////////////////////////////////////////////////////////////////////////////////
// Application designed to read and decode telemetry from a Piccolo autopilot via //
// Piccolo Commnad Center server function.  The decoded data is then provided to  //
// exterior clients via a TCP server.											  //
////////////////////////////////////////////////////////////////////////////////////
...
#define SERVER "192.168.10.102"	//IP address of UDP server
#define BUFLEN 1024  //Max length of buffer
#define PORT 49011	//The port on which to listen for incoming data
...
// Structure defining the messages to be sent to Overwatch Aero application
// 111 bytes + 3 bytes ("GPS") = 114 bytes
struct OverwatchGPSMessage
{
	double	Latitude;
	double	Longitude;
	double	Altitude;
...
// 175 bytes + 3 bytes ("TEL") = 178 bytes
struct OverwatchTelemMessage
{
	UInt32	SystemTime;
	float	Euler_Roll;
	float	Euler_Pitch;
	...
	BOOL	Electric;
	float	AlignHeading;
	float	AlignHeadingSigma;
	UInt8	AlignSolnType;
	UInt8	AlignVisibleSats;
	UInt8	AlignTrackedSats;
	float	PRes_X;
	float	PRes_Y;
	float	PRes_Z;
	float	VRes_X;
...
// Local functions
static char* PassGPSToOverwatch(UserData_t* pData, char buffer[]);
static char* PassTelToOverwatch(UserData_t* pData, char buffer[]);
static char* PassSysToOverwatch(UserData_t* pData, char buffer[]);
static void RunCommunications(CommBase_t* pComm, FILE* pTextLog);
static BOOL ProcessKeyboard(CommBase_t* pComm);
static void RegularService(CommBase_t* pComm);
...
static void ProcessMovingBaseStatus(const AutopilotPkt_t* pPkt);
static void ProcessServoTelemetry(const AutopilotPkt_t* pPkt);
...
int main(int argc, char* argv[])
{
	CommBase_t Comm;
	char DevicePath[64] = "";
	char Server[128] = "localhost:2001";
	char ReplayFile[512] = "";
	FILE* pTextLog = NULL;
	BOOL Replay = FALSE;
	int ListenPort = -1;
	...
	printf("\n\rOverwatch Aero, LLC Piccolo Export Program");
	printf("\n\rVERSION == 0.1\n\r\n\r");
	printf("Switches [-P=x] [-B=x] [-DEV=devicepath] [-SERVER=server:port] [-SN=serialnumber] [-LISTEN=port]\n\r");
	printf("-P=x  the serial port number, default of 1\n\r");
	printf("-B=x  the serial port baud rate, default of 57600\n\r");
	printf("-DEV=devicepath the Unix communication device path, for example \"/dev/ttyS0\"\n\r");
	...
	for (i = 1; i < argc; i++)
	{
		...
		param = strstr(argv[i], "-DEV=");
		if (param)
		{
			param += 5;				// Move forward past parameter name
			strcpy(DevicePath, param);	// Get the server name in the parameter
			continue;				// Move to next parameter
	...
	if (Replay)
		CommBase_OpenReplay(&Comm, ReplayFile, ListenPort);

	else if (Port < 0xFF)
		CommBase_OpenSerial(&Comm, Port, Baud, NULL, ListenPort, TRUE, NULL, FALSE);

	else if (strlen(DevicePath) > 0)
		CommBase_OpenDevice(&Comm, DevicePath, Baud, NULL, ListenPort, TRUE, NULL, FALSE);

	else
		CommBase_OpenSocket(&Comm, Server, TRUE, NULL, FALSE);

	if (CommBase_GetLastError(&Comm))
		printf("%s", CommBase_GetLastError(&Comm));

	// If we are doing a replay then tell the speed
	if (Replay)
		CommBase_SetReplayRate(&Comm, ReplayRate);

	// Register new avionics callback function
	CommBase_SetNewNetworkCallBack(&Comm, NewNetwork, 0);

	// Run the main communications function
	RunCommunications(&Comm, pTextLog);
 ...
 *         will be done*/
void RunCommunications(CommBase_t* pComm, FILE* pTextLog)
{
	...
	//----------------------------------------------------------
	// Overwatch Aero, LLC Socket
	// Reference:  https://www.youtube.com/watch?v=WDn-htpBlnU
	//----------------------------------------------------------
	//Initialise winsock
	ver = MAKEWORD(2, 2);			// Winsock version to build
	...
	{
		printf("Can't create valid TCP server socket!");
	}

	// Bind the ip address and port for the TCP server to the socket
	struct sockaddr_in tcp_server_socket;
	tcp_server_socket.sin_family = AF_INET;
	tcp_server_socket.sin_port = htons(54000);
	tcp_server_socket.sin_addr.S_un.S_addr = INADDR_ANY;

	bind(listening, (struct sockaddr*)&tcp_server_socket, sizeof(tcp_server_socket));
	listen(listening, SOMAXCONN);

	// Wait for connection to this address
	struct sockaddr_in client;
	int clientSize = sizeof(client);

	printf("Listening for client TCP connections on: %i", htons(tcp_server_socket.sin_port));
	SOCKET clientSocket = accept(listening, (struct sockaddr*)&client, &clientSize);
	if (clientSocket == INVALID_SOCKET)
	{
		printf("Can't create valid client socket!");
	}

	// Changes the socket into a non-blocking state
	fcntl()
	
	char host[NI_MAXHOST];		// Client's remote name
	char service[NI_MAXHOST];	// Service (i.e. port) the client is connected on

	ZeroMemory(host, NI_MAXHOST);		// Same as memset(host, 0, NIMAXHOST);
	...
	// Close listenting TCP socket
	// CHANGE EVENTUALLY AFTER INITIAL TESTING TO ACCEPT NEW CONNECTIONS
	...
	// SerialNumber is set to 0 by default, unless provided as a parameter
	...
	if (SerialNumber == 0)
	{
		...
		// Loop waits to connect to an autopilot.  Checks each itteration
		//   the user has tried to quit or not.
	...
	{
		CommBase_CreateNetwork(pComm, SerialNumber);
		printf("Press Q to quit\n\r");
	...
	while (1)
	{
		...
		else
		{
			...
			while (LookForAutopilotPacket(pRxQ, &Pkt))
			{
				if (ProcessAutopilotPacket(&Pkt, &Data, &LeftECU, &RightECU))
				{
					...
					if (pTextLog)
						CommBase_WriteTextLogLine(pTextLog, &Data, AckRatio, NULL, NULL, NULL, &LeftECU, &RightECU, NULL, NULL,
							GSData.System.RSSI, GSData.PilotSamplePercentage, GSData.PilotUplinkRate);

				}// If decoded telemetry packet

				// ***********************************************
				// Begin OVERWATCH UDP socket send
				// ***********************************************
				//Start communication
				// Send GPS data
				memset(sendData, 0, sizeof(sendData));
				char* strGPS = PassGPSToOverwatch(&Data, sendData);
				if (sendto(s, strGPS, strlen(strGPS), 0, (struct sockaddr*)&si_other, sizeof(si_other)) == SOCKET_ERROR)
				{
				...
				// Send Telemetry data
				memset(sendData, 0, sizeof(sendData));
				char* strTel = PassTelToOverwatch(&Data, sendData);
				if (sendto(s, strTel, strlen(strTel), 0, (struct sockaddr*)&si_other, sizeof(si_other)) == SOCKET_ERROR)
				{
					printf("sendto() failed with error code: %d", WSAGetLastError());
					exit(EXIT_FAILURE);
				}
				// Send System data
				memset(sendData, 0, sizeof(sendData));
				char* strSys = PassTelToOverwatch(&Data, sendData);
				if (sendto(s, strSys, strlen(strSys), 0, (struct sockaddr*)&si_other, sizeof(si_other)) == SOCKET_ERROR)
				{
					printf("sendto() failed with error code: %d", WSAGetLastError());
					exit(EXIT_FAILURE);
				}
				// ***********************************************
				// End OVERWATCH socket send
				// ***********************************************
		...
		if (!pGSRxQ)
			pGSRxQ = CommBase_GetStreamRxBuffer(pComm, GS_ADDRESS, GS_STREAM);
		else
		{
	...
	// Close OVERWATCH socket
	closesocket(s);
	WSACleanup();
}// RunCommunications

// Method to send Piccolo GPS data to Overwatch Aero programs via UDP
char* PassGPSToOverwatch(UserData_t* pData, char buffer[])
{
	char msgType[3];

	// Overwatch GPS Message
	strcpy(msgType, "GPS");
	sprintf(buffer, "%s,%0.10lf,%0.10lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
		msgType, pData->GPS.Latitude, pData->GPS.Longitude, pData->GPS.Altitude,
		pData->GPS.PosECEF[X_AXIS], pData->GPS.PosECEF[Y_AXIS], pData->GPS.PosECEF[Z_AXIS],
		pData->GPS.VNorth, pData->GPS.VEast, pData->GPS.VDown,
		pData->GPS.VelECEF[X_AXIS], pData->GPS.VelECEF[Y_AXIS], pData->GPS.VelECEF[Z_AXIS],
		pData->GPS.Direction, pData->GPS.Speed, pData->GPS.PDOP,
		pData->GPS.seconds, pData->GPS.Accuracy, pData->GPS.ITOW,
		pData->GPS.Week, pData->GPS.Status, pData->GPS.year,
		pData->GPS.month, pData->GPS.day, pData->GPS.hours,
		pData->GPS.minutes, pData->GPS.VisibleSats, pData->GPS.TrackedSats,
		pData->GPS.SolnType, pData->GPS.UsedInINS, pData->GPS.GPSType);

	return buffer;
}

// Method to send Piccolo Telemetry data to Overwatch Aero programs via UDP
char* PassTelToOverwatch(UserData_t* pData, char buffer[])
{
	char msgType[3];

	// Overwatch GPS Message
	strcpy(msgType, "TEL");
	sprintf(buffer, "%s,%u,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%u,%u,%0.6lf,%u,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%u,%0.6lf,%0.6lf,%u,%u,%u,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.10lf,%0.10lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
		msgType, pData->SystemTime, pData->Euler[ROLL_AXIS], pData->Euler[PITCH_AXIS], pData->Euler[YAW_AXIS],
		pData->BaroAlt, pData->TAS, pData->Density, pData->WindSouth, pData->WindWest,
		pData->LeftRPM, pData->RightRPM, pData->WeightOnWheel, pData->GroundContact ? 1 : 0,
		pData->IAS, pData->Pdynamic, pData->OAT, pData->Pstatic,
		pData->Gyro[ROLL_AXIS], pData->Gyro[PITCH_AXIS], pData->Gyro[YAW_AXIS],
		pData->Accel[X_AXIS], pData->Accel[Y_AXIS], pData->Accel[Z_AXIS],
		pData->MagField[X_AXIS], pData->MagField[Y_AXIS], pData->MagField[Z_AXIS],
		pData->MagHeading, pData->AGL, pData->Actuators[0], pData->Actuators[1],
		pData->Actuators[2], pData->Actuators[3], pData->Actuators[4],
		pData->Actuators[5], pData->Actuators[6], pData->Actuators[7],
		pData->Fuel, pData->FuelFlow, pData->Electric ? 1 : 0, pData->AlignHeading,
		pData->AlignHeadingSigma, pData->AlignSolnType, pData->AlignVisibleSats,
		pData->AlignTrackedSats, pData->PRes[X_AXIS], pData->PRes[Y_AXIS], pData->PRes[Z_AXIS],
		pData->VRes[X_AXIS], pData->VRes[Y_AXIS], pData->VRes[Z_AXIS],
		pData->AltUsed, pData->GPS.Latitude, pData->GPS.Longitude, pData->GPS.Altitude,
		pData->GPS.PosECEF[X_AXIS], pData->GPS.PosECEF[Y_AXIS], pData->GPS.PosECEF[Z_AXIS],
		pData->GPS.VNorth, pData->GPS.VEast, pData->GPS.VDown,
		pData->GPS.VelECEF[X_AXIS], pData->GPS.VelECEF[Y_AXIS], pData->GPS.VelECEF[Z_AXIS],
		pData->GPS.Direction, pData->GPS.Speed, pData->GPS.PDOP,
...
// Method to send Piccolo System data to Overwatch Aero programs via UDP
char* PassSysToOverwatch(UserData_t* pData, char buffer[])
{
	char msgType[3];

	// Overwatch GPS Message
	strcpy(msgType, "SYS");
	sprintf(buffer, "%s,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf,%0.6lf",
		msgType, pData->System.InputVoltage, pData->System.InputCurrent, pData->System.ServoVoltage, pData->System.ServoCurrent,
		pData->System.BoardTemp, pData->System.RSSI, pData->System.VSWR, pData->System.FirstStageBIT, pData->System.FiveDigitalBIT,
		pData->System.FiveAnalogBIT, pData->System.CPUVoltageBIT, pData->System.GPSVoltageBIT, pData->System.RadioVoltageBIT,
		pData->System.TempBIT, pData->System.TempDisagreeBIT, pData->System.NoInternalGPS, pData->System.PrimaryIMUBIT,
		pData->System.SecondaryIMUBIT, pData->System.TertiaryIMUBIT, pData->System.PrimaryPdynamicBIT, pData->System.PrimaryPstaticBIT,
		pData->System.PrimaryAirDataReversed, pData->System.SecondaryPdynamicBIT, pData->System.SecondaryPstaticBIT,
		pData->System.SecondaryAirDataReversed, pData->System.TertiaryPdynamicBIT, pData->System.TertiaryPstaticBIT,
		pData->System.TertiaryAirDataReversed, pData->System.Reserved, pData->System.CPULoad, pData->System.UplinkRatio,
		pData->System.VibrationX, pData->System.VibrationY, pData->System.VibrationZ, pData->System.VibrationP,
		pData->System.VibrationQ, pData->System.VibrationR);

	return buffer;
}

// Method to send Piccolo NavDiagnostics data to Overwatch Aero programs via UDP
/*char* PassNavToOverwatch(UserData_t* pData, char buffer[])
{
	char msgType[3];
...
BOOL ProcessKeyboard(CommBase_t* pComm)
{
	while (_kbhit())
	{
		int key = _getch();
		...
		case 224:	// arrow keys
		{
			UInt32 Rate = CommBase_GetReplayRate(pComm);
			...
			// Set the new rate
			CommBase_SetReplayRate(pComm, Rate);
...
BOOL ProcessAutopilotPacket(AutopilotPkt_t* pPkt, UserData_t* pData, ECU_t* pLeftECU, ECU_t* pRightECU)
{
	...
	case MOVING_BASELINE_STATUS: //!< Moving base status data
		ProcessMovingBaseStatus(pPkt);
		break;

	case SERVO_TELEMETRY:
		ProcessServoTelemetry(pPkt);
...
BOOL ProcessTelemetry(const AutopilotPkt_t* pPkt, UserData_t* pData)
{
	...
	if (Return)
	{
		...
		printf("Pos: %0.6lf %0.6lf %0.2lf Sats:%d PDOP:%0.1f %d:%d:%0.2f\n\r",
			pData->GPS.Latitude * 180.0 / PI,	// radians to degrees
...
void ProcessSystemVersion(const AutopilotPkt_t* pPkt)
{
	...
	if (DecodeSystemVersionPacket(pPkt, &Version, &Board))
	{
		...
		Type = (UInt8)((Board.Config & RADIO_CONFIG_BITS) >> 20);
		switch (Type)
		{
...
/*! This is the helper function that processes a moving baseline status packet.
	\param pPkt points to valid autopilot packet received from an avionics.
*/
void ProcessMovingBaseStatus(const AutopilotPkt_t* pPkt)
{
	MovingBaseStatus_t MBStatus;

	if (DecodeMovingBaseStatusPacket(pPkt, &MBStatus))
	{
		printf("MOVING_BASELINE_STATUS packet:\n\r");
		printf("Baseline North, East, Down: %8.2f, %8.2f, %8.2f m.\r\n", MBStatus.Baseline[NORTH], MBStatus.Baseline[EAST], MBStatus.Baseline[DOWN]);
		printf("Error: %8.2f m, lag: %6u ms\r\n", MBStatus.Error, MBStatus.Lag);
		printf("Soln type: %3u", MBStatus.SolnType);
		if (MBStatus.IsPPS)
			printf(" SAASM");
		printf(".\r\n");
	}
	else
		printf("Invalid MOVING_BASELINE_STATUS packet\n\r");

	printf("\n\r");

} // ProcessMovingBaseStatus


/*! This is the helper function that processes a moving baseline status packet.
	\param pPkt points to valid autopilot packet received from an avionics.
*/
void ProcessServoTelemetry(const AutopilotPkt_t* pPkt)
{
	ServoTelemetry_t ServoTel[NUM_ACTUATORS];
	UInt32 i, NumServos;

	// Read the servo telemetry from the packet
	NumServos = DecodeServoTelemetryPacket(pPkt, ServoTel);

	if (NumServos)
	{
		printf("SERVO_TELEMETRY packet:\n\r");

		// Print out the servo telemetry to the screen
		for (i = 0; i < NumServos; i++)
		{
			printf("Servo %d:\r\n", ServoTel[i].Channel);
			printf("    Mode %d, avg/max current %d/%d, temp %d\r\n", ServoTel[i].Mode,
				ServoTel[i].AvgCurrent, ServoTel[i].MaxCurrent, ServoTel[i].Temperature);
			printf("    Pos %d, pos err %d, err bits %d\r\n", ServoTel[i].Position,
				ServoTel[i].PosError, ServoTel[i].ErrorBits);

		}
	}
	else
		printf("Invalid SERVO_TELEMETRY packet.\r\n");

	printf("\r\n");

} // ProcessServoTelemetry
